...
import {getCategoryProducts, getCategoryProductsCount } from '../../functions/category'
...
const CategoryHome = ({match}) => {
  ...
  const loadCategoryProducts = useCallback(() => {
    setLoading(true)
    getCategoryProducts(slug, 'createdAt', 'desc', page)
    .then(res =>{
  ...
  return (
    ...
    {loading ? (<Spinner />) : (
      ...
      {
        products.length === 0 && (
          <div className=" mx-auto d-flex flex-column align-items-center"  style={{width:'40%'}}>
          <h5 className="text-center mt-3 mb-5  ">
        No products in this category
      </h5>
      <div
      onClick={() => history.push('/')}
      className="btn unique" style={{width:'100px'}}> Home</div>
      </div>
        )
      }
      
        <div className="container mb-3 mt-5 ginek" >
        
          <div className="row heniek">
          {products.map(product => (
            <div className="col-md-4 d-flex align-items-stretch" key={product._id}>
